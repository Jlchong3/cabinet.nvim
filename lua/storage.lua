...
---@param filename string
M.get_data_file_name = function(filename)
...
---@param tbl table
M.save_data = function(tbl)
    ensure_dir(M.data_path)

    local filename = M.get_data_file_name(vim.fn.getcwd()) -- filename is based on cwd
    ...
    f:write(M.serialize(tbl))
    f:close()
end

M.load_table = function ()
    local filename = M.get_data_file_name(vim.fn.getcwd())
    local data_file_path = create_file_path(M.data_path, filename)
